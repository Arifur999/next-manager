...
import {
  addClientLinkAction,
  removeClientLinkAction,
} from "@/app/(dashboardLayout)/admin/dashboard/clients/_linkAction"
import { Button } from "@/components/ui/button"
import { Card, CardHeader, CardTitle } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { getClientLinks } from "@/services/agencio.services"
...
/**
 * Where this client's work actually lives.
 *
...
const ClientLinksPanel = ({ clientId }: { clientId: string }) => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getClientLinks(`client_id=${clientId}`),
  })

  const links = (data?.data ?? []) as IClientLink[]

  const refresh = () => {
    void queryClient.invalidateQueries({ queryKey: ["client-links", clientId] })
  }

  const { mutate: add, isPending } = useMutation({
    mutationFn: () =>
      addClientLinkAction({ client_id: clientId, label: label.trim(), url: url.trim() }),
    onSuccess: (result) => {
      if (!result.success) {
  ...
  const { mutate: remove, isPending: isRemoving } = useMutation({
    mutationFn: (id: string) => removeClientLinkAction(id),
    onSuccess: (result) => {
      if (!result.success) {
    ...
    <Card className="gap-0 overflow-hidden p-0">
      <CardHeader className="border-b px-5 py-4">
        ...
        <p className="text-sm text-muted-foreground">
          Drive folders, Figma files, staging sites — wherever this client&apos;s work
          actually lives.
        </p>
      </CardHeader>
...
export default ClientLinksPanel
